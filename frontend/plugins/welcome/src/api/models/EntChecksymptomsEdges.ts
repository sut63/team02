/* tslint:disable */
/* eslint-disable */
/**
 * SUT SA Example API
 * This is a sample server for SUT SE 2563
 *
 * The version of the OpenAPI document: 1.0
 * Contact: support@swagger.io
 *
 * NOTE: This class is auto generated by OpenAPI Generator (https://openapi-generator.tech).
 * https://openapi-generator.tech
 * Do not edit the class manually.
 */

import { exists, mapValues } from '../runtime';
import {
    EntDisease,
    EntDiseaseFromJSON,
    EntDiseaseFromJSONTyped,
    EntDiseaseToJSON,
    EntDoctorOrderSheet,
    EntDoctorOrderSheetFromJSON,
    EntDoctorOrderSheetFromJSONTyped,
    EntDoctorOrderSheetToJSON,
    EntPatient,
    EntPatientFromJSON,
    EntPatientFromJSONTyped,
    EntPatientToJSON,
    EntPersonnel,
    EntPersonnelFromJSON,
    EntPersonnelFromJSONTyped,
    EntPersonnelToJSON,
} from './';

/**
 * 
 * @export
 * @interface EntChecksymptomsEdges
 */
export interface EntChecksymptomsEdges {
    /**
     * 
     * @type {EntDisease}
     * @memberof EntChecksymptomsEdges
     */
    disease?: EntDisease;
    /**
     * 
     * @type {EntDoctorOrderSheet}
     * @memberof EntChecksymptomsEdges
     */
    doctorordersheet?: EntDoctorOrderSheet;
    /**
     * 
     * @type {EntPatient}
     * @memberof EntChecksymptomsEdges
     */
    patient?: EntPatient;
    /**
     * 
     * @type {EntPersonnel}
     * @memberof EntChecksymptomsEdges
     */
    personnel?: EntPersonnel;
}

export function EntChecksymptomsEdgesFromJSON(json: any): EntChecksymptomsEdges {
    return EntChecksymptomsEdgesFromJSONTyped(json, false);
}

export function EntChecksymptomsEdgesFromJSONTyped(json: any, ignoreDiscriminator: boolean): EntChecksymptomsEdges {
    if ((json === undefined) || (json === null)) {
        return json;
    }
    return {
        
        'disease': !exists(json, 'disease') ? undefined : EntDiseaseFromJSON(json['disease']),
        'doctorordersheet': !exists(json, 'doctorordersheet') ? undefined : EntDoctorOrderSheetFromJSON(json['doctorordersheet']),
        'patient': !exists(json, 'patient') ? undefined : EntPatientFromJSON(json['patient']),
        'personnel': !exists(json, 'personnel') ? undefined : EntPersonnelFromJSON(json['personnel']),
    };
}

export function EntChecksymptomsEdgesToJSON(value?: EntChecksymptomsEdges | null): any {
    if (value === undefined) {
        return undefined;
    }
    if (value === null) {
        return null;
    }
    return {
        
        'disease': EntDiseaseToJSON(value.disease),
        'doctorordersheet': EntDoctorOrderSheetToJSON(value.doctorordersheet),
        'patient': EntPatientToJSON(value.patient),
        'personnel': EntPersonnelToJSON(value.personnel),
    };
}


