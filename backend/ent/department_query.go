...
func (dq *DepartmentQuery) QueryPersonnel() *PersonnelQuery {
...
func (dq *DepartmentQuery) WithPersonnel(opts ...func(*PersonnelQuery)) *DepartmentQuery {
	query := &PersonnelQuery{config: dq.config}
...
func (dq *DepartmentQuery) sqlAll(ctx context.Context) ([]*Department, error) {
	...
	if query := dq.withPersonnel; query != nil {
		...
		for i := range nodes {
			...
			nodes[i].Edges.Personnel = []*Personnel{}
		}
		query.withFKs = true
		query.Where(predicate.Personnel(func(s *sql.Selector) {
			s.Where(sql.InValues(department.PersonnelColumn, fks...))
		}))
		...
		for _, n := range neighbors {
			fk := n._Department
			if fk == nil {
				return nil, fmt.Errorf(`foreign-key "_Department" is nil for node %v`, n.ID)
			}
			node, ok := nodeids[*fk]
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "_Department" returned %v for node %v`, *fk, n.ID)
			}
			node.Edges.Personnel = append(node.Edges.Personnel, n)
		}
	}
