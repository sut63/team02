...
func (sq *StatusQuery) QueryPhysicaltherapyrecord() *PhysicaltherapyrecordQuery {
	query := &PhysicaltherapyrecordQuery{config: sq.config}
...
func (sq *StatusQuery) Clone() *StatusQuery {
	...
	return &StatusQuery{
		...
		withPhysicaltherapyrecord: sq.withPhysicaltherapyrecord.Clone(),
...
func (sq *StatusQuery) WithPhysicaltherapyrecord(opts ...func(*PhysicaltherapyrecordQuery)) *StatusQuery {
	query := &PhysicaltherapyrecordQuery{config: sq.config}
	for _, opt := range opts {
		opt(query)
	}
	sq.withPhysicaltherapyrecord = query
	return sq
}
...
func (sq *StatusQuery) sqlAll(ctx context.Context) ([]*Status, error) {
	var (
		...
		_spec       = sq.querySpec()
		loadedTypes = [1]bool{
			sq.withPhysicaltherapyrecord != nil,
		}
	)
	...
	if query := sq.withPhysicaltherapyrecord; query != nil {
		...
		query.Where(predicate.Physicaltherapyrecord(func(s *sql.Selector) {
			s.Where(sql.InValues(status.PhysicaltherapyrecordColumn, fks...))
		}))
		...
		for _, n := range neighbors {
			fk := n.status_physicaltherapyrecord
			if fk == nil {
				return nil, fmt.Errorf(`foreign-key "status_physicaltherapyrecord" is nil for node %v`, n.ID)
			}
			node, ok := nodeids[*fk]
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "status_physicaltherapyrecord" returned %v for node %v`, *fk, n.ID)
			}
...
func (ss *StatusSelect) Scan(ctx context.Context, v interface{}) error {
	...
	ss.sql = ss.StatusQuery.sqlQuery()
