...
func (pq *PatientQuery) QueryChecksymptom() *ChecksymptomQuery {
...
func (pq *PatientQuery) QueryDentalappointment() *DentalappointmentQuery {
...
func (pq *PatientQuery) WithChecksymptom(opts ...func(*ChecksymptomQuery)) *PatientQuery {
	query := &ChecksymptomQuery{config: pq.config}
	for _, opt := range opts {
		opt(query)
	}
...
func (pq *PatientQuery) WithDentalappointment(opts ...func(*DentalappointmentQuery)) *PatientQuery {
	query := &DentalappointmentQuery{config: pq.config}
	for _, opt := range opts {
		opt(query)
	}
...
func (pq *PatientQuery) sqlAll(ctx context.Context) ([]*Patient, error) {
	...
	if query := pq.withChecksymptom; query != nil {
		...
		query.Where(predicate.Checksymptom(func(s *sql.Selector) {
			s.Where(sql.InValues(patient.ChecksymptomColumn, fks...))
		}))
		neighbors, err := query.All(ctx)
		if err != nil {
			return nil, err
	...
	if query := pq.withDentalappointment; query != nil {
		...
		query.Where(predicate.Dentalappointment(func(s *sql.Selector) {
			s.Where(sql.InValues(patient.DentalappointmentColumn, fks...))
		}))
		neighbors, err := query.All(ctx)
		if err != nil {
			return nil, err
	...
	if query := pq.withAntenatalinformation; query != nil {
		...
		query.Where(predicate.Antenatalinformation(func(s *sql.Selector) {
			s.Where(sql.InValues(patient.AntenatalinformationColumn, fks...))
		}))
		neighbors, err := query.All(ctx)
		if err != nil {
			return nil, err
