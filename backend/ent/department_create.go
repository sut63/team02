...
func (dc *DepartmentCreate) AddPersonnelIDs(ids ...int) *DepartmentCreate {
	dc.mutation.AddPersonnelIDs(ids...)
	return dc
}

// AddPersonnel adds the "Personnel" edges to the Personnel entity.
func (dc *DepartmentCreate) AddPersonnel(p ...*Personnel) *DepartmentCreate {
	ids := make([]int, len(p))
	for i := range p {
		ids[i] = p[i].ID
	}
	return dc.AddPersonnelIDs(ids...)
}
